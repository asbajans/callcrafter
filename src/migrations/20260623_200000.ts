...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  // Indexes (some columns may not exist if 20260623_220000 created the tables with different names)
  ...
  await db.execute(sql`DO $$ BEGIN IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tenant_credits' AND column_name = 'tenant_id') THEN CREATE INDEX IF NOT EXISTS "tenant_credits_tenant_idx" ON "tenant_credits" USING btree ("tenant_id"); END IF; END $$;`)
  ...
  await db.execute(sql`DO $$ BEGIN IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'credit_transactions' AND column_name = 'tenant_id') THEN CREATE INDEX IF NOT EXISTS "credit_transactions_tenant_idx" ON "credit_transactions" USING btree ("tenant_id"); END IF; END $$;`)
  ...
  await db.execute(sql`DO $$ BEGIN IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'usage_logs' AND column_name = 'tenant_id') THEN CREATE INDEX IF NOT EXISTS "usage_logs_tenant_idx" ON "usage_logs" USING btree ("tenant_id"); END IF; END $$;`)
  ...
  await db.execute(sql`DO $$ BEGIN IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tenant_provider_access' AND column_name = 'tenant_id') THEN CREATE INDEX IF NOT EXISTS "tenant_provider_access_tenant_idx" ON "tenant_provider_access" USING btree ("tenant_id"); END IF; END $$;`)
  await db.execute(sql`DO $$ BEGIN IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tenant_provider_access' AND column_name = 'provider_id') THEN CREATE INDEX IF NOT EXISTS "tenant_provider_access_provider_idx" ON "tenant_provider_access" USING btree ("provider_id"); END IF; END $$;`)
